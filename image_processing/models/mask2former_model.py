...
from transformers.file_utils import (ModelOutput, is_scipy_available,
                                     requires_backends)
...
from transformers.utils import auto_docstring, is_accelerate_available, logging
...
def _copy_init_(tensor: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
    with torch.no_grad():
...
@auto_docstring(custom_intro="""
    ...
    """)
class Mask2FormerMaskedAttentionDecoderOutput(
        BaseModelOutputWithCrossAttentions):
    r"""
...
class Mask2FormerModelOutput(ModelOutput):
    ...
    pixel_decoder_last_hidden_state (`torch.FloatTensor` of shape
        `(batch_size, num_channels, height, width)`, *optional*):
    ...
    pixel_decoder_hidden_states (`tuple(torch.FloatTensor)`, *optional*,
        returned when `output_hidden_states=True` or `config.output_hidden_states=True`):
        Tuple of `torch.FloatTensor` (one for the output of the embeddings + one for the output of each stage) of
    ...
    transformer_decoder_intermediate_states: tuple[
...
class Mask2FormerForUniversalSegmentationOutput(ModelOutput):
    ...
    attentions (`tuple(tuple(torch.FloatTensor))`, *optional*,
        returned when `output_attentions=True` or `config.output_attentions=True`):
        Tuple of `tuple(torch.FloatTensor)` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
...
def sample_point(input_features: torch.Tensor,
...
def sigmoid_cross_entropy_loss(inputs: torch.Tensor, labels: torch.Tensor,
...
class Mask2FormerHungarianMatcher(nn.Module):
    ...
    def forward(
        ...
        for i in range(batch_size):
            ...
            point_coordinates = torch.rand(1,
                                           self.num_points,
                                           2,
            ...
            target_mask = sample_point(target_mask,
                                       target_coordinates,
                                       align_corners=False).squeeze(1)

            pred_coordinates = point_coordinates.repeat(
                pred_mask.shape[0], 1, 1)
            pred_mask = sample_point(pred_mask,
                                     pred_coordinates,
                                     align_corners=False).squeeze(1)

            # compute the cross entropy loss between each mask pairs -> shape (num_queries, num_labels)
            cost_mask = pair_wise_sigmoid_cross_entropy_loss(
...
class Mask2FormerLoss(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig, weight_dict: dict[str,
                                                                    float]):
        """
    ...
    def _build_focal_alpha_weights(self) -> torch.Tensor:
        if self.focal_class_alphas is None:
            alpha_weights = torch.full((self.num_labels + 1, ),
        ...
        alpha_weights = torch.as_tensor(self.focal_class_alphas,
                                        dtype=torch.float32)
        if alpha_weights.numel() == self.num_labels:
            no_object_alpha = self.focal_no_object_alpha
            if no_object_alpha is None:
                no_object_alpha = 1.0 - float(self.focal_alpha)
        ...
        elif alpha_weights.numel() != self.num_labels + 1:
            raise ValueError(
                "focal_class_alphas must have length num_labels or num_labels + 1. "
                f"Got {alpha_weights.numel()} for num_labels={self.num_labels}."
            )
    ...
    def _pad_images_to_max_in_batch(
            self, tensors: list[Tensor]) -> tuple[Tensor, Tensor]:
        # get the maximum size in the batch
        max_size = self._max_by_axis(
        ...
        padding_masks = torch.ones((batch_size, height, width),
                                   dtype=torch.bool,
                                   device=device)
        # pad the tensors to the size of the biggest one
        for tensor, padded_tensor, padding_mask in zip(tensors, padded_tensors,
                                                       padding_masks):
            padded_tensor[:tensor.shape[0], :tensor.shape[1], :tensor.
                          shape[2]].copy_(tensor)
            padding_mask[:tensor.shape[1], :tensor.shape[2]] = False

        return padded_tensors, padding_masks

    # def loss_labels( #loss
    ...
    #         - **loss_cross_entropy** -- The loss computed using cross entropy
    #           on the predicted and ground truth labels.
    #     """
    #     pred_logits = class_queries_logits
    #     batch_size, num_queries, _ = pred_logits.shape
    #     criterion = nn.CrossEntropyLoss(weight=self.empty_weight)
    #     idx = self._get_predictions_permutation_indices(indices)  # shape of (batch_size, num_queries)
    #     target_classes_o = torch.cat(   #ce 생성
    ...
    #     losses = {"loss_cross_entropy": loss_ce}  #loss_ce (cross entropy loss)  ce 계산
    #     return losses
    ...
    def loss_labels(
        ...
        if self.classification_loss_type == "seesaw":
            ...
            loss_ce = softmax_focal_loss(
                ...
                alpha=self.focal_alpha_weights.to(pred_logits.device),
    ...
    def loss_masks(
        ...
        with torch.no_grad():
            ...
            point_labels = sample_point(target_masks,
                                        point_coordinates,
                                        align_corners=False).squeeze(1)

        point_logits = sample_point(pred_masks,
                                    point_coordinates,
                                    align_corners=False).squeeze(1)
        losses = {
            "loss_mask":
            sigmoid_cross_entropy_loss(point_logits, point_labels, num_masks),
            "loss_dice":
            dice_loss(point_logits, point_labels, num_masks),
        }
    ...
    def calculate_uncertainty(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            logits (`torch.Tensor`):
            A tensor of shape (R, 1, ...) for class-specific or class-agnostic,
            where R is the total number of predicted masks in all images and C is:
    ...
    def sample_points_using_uncertainty(
        ...
        point_coordinates = torch.rand(num_boxes,
                                       num_points_sampled,
                                       2,
                                       device=logits.device)
        # Get sampled prediction value for the point coordinates
        point_logits = sample_point(logits,
                                    point_coordinates,
                                    align_corners=False)
        ...
        point_coordinates = point_coordinates.view(
            -1, 2)[idx.view(-1), :].view(num_boxes, num_uncertain_points, 2)
    ...
    def forward( #loss_masks + loss_labels
        ...
        indices = self.matcher(masks_queries_logits, class_queries_logits,
                               mask_labels, class_labels)
        # compute the average number of target masks for normalization purposes
        num_masks = self.get_num_masks(class_labels,
                                       device=class_labels[0].device)
        # get all the losses
...
def multi_scale_deformable_attention(
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        # -> batch_size*num_heads, hidden_dim, height, width
        value_l_ = (value_list[level_id].flatten(2).transpose(1, 2).reshape(
            batch_size * num_heads, hidden_dim, height, width))
        # batch_size, num_queries, num_heads, num_points, 2
        # -> batch_size, num_heads, num_queries, num_points, 2
        # -> batch_size*num_heads, num_queries, num_points, 2
        sampling_grid_l_ = sampling_grids[:, :, :,
                                          level_id].transpose(1,
                                                              2).flatten(0, 1)
        # batch_size*num_heads, hidden_dim, num_queries, num_points
        sampling_value_l_ = nn.functional.grid_sample(value_l_,
                                                      sampling_grid_l_,
                                                      mode="bilinear",
                                                      padding_mode="zeros",
                                                      align_corners=False)
    ...
    output = ((torch.stack(sampling_value_list, dim=-2).flatten(-2) *
               attention_weights).sum(-1).view(batch_size,
...
# .MaskFormerSinePositionEmbedding with MaskFormer->Mask2Former
class Mask2FormerSinePositionEmbedding(nn.Module):
    ...
    def forward(
        ...
        if mask is None:
            mask = torch.zeros((shape[0], shape[2], shape[3]),
                               device=device,
                               dtype=torch.bool)
...
class Mask2FormerPixelDecoderEncoderMultiscaleDeformableAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, n_levels: int,
                 n_points: int):
        super().__init__()
        ...
        if not ((dim_per_head &
                 (dim_per_head - 1) == 0) and dim_per_head != 0):
            warnings.warn(
        ...
        self.sampling_offsets = nn.Linear(embed_dim,
    ...
    def forward(
        ...
        if position_embeddings is not None:
            hidden_states = self.with_pos_embed(hidden_states,
                                                position_embeddings)

        batch_size, num_queries, _ = hidden_states.shape
        batch_size, sequence_length, _ = encoder_hidden_states.shape
        total_elements = sum(height * width
        ...
        value = value.view(batch_size, sequence_length, self.n_heads,
                           self.d_model // self.n_heads)
        sampling_offsets = self.sampling_offsets(hidden_states).view(
            batch_size, num_queries, self.n_heads, self.n_levels,
            self.n_points, 2)
        attention_weights = self.attention_weights(hidden_states).view(
            batch_size, num_queries, self.n_heads,
        ...
        output = multi_scale_deformable_attention(value, spatial_shapes_list,
                                                  sampling_locations,
                                                  attention_weights)
...
# Modified from transformers.models.detr.modeling_deformable_detr
# .DeformableDetrEncoder with DeformableDetrEncoder->Mask2FormerPixelDecoderEncoderOnly
...
# Modified from transformers.models.detr.modeling_deformable_detr
# .DeformableDetrModel with DeformableDetrModel->Mask2FormerPixelDecoder
    ...
    def __init__(self, config: Mask2FormerConfig, feature_channels):
        ...
        if self.num_feature_levels > 1:
            ...
            self.input_projections = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(transformer_in_channels[-1],
                              feature_dim,
                              kernel_size=1),
                    nn.GroupNorm(32, feature_dim),
                )
        ...
        self.mask_projection = nn.Conv2d(feature_dim,
                                         mask_dim,
                                         kernel_size=1,
                                         stride=1,
                                         padding=0)
    ...
    def forward(
        ...
        masks = [
            torch.zeros((x.size(0), x.size(2), x.size(3)),
                        device=x.device,
                        dtype=torch.bool) for x in inputs_embeds
        ]

        # Prepare encoder inputs (by flattening)
        spatial_shapes_list = [(embed.shape[2], embed.shape[3])
                               for embed in inputs_embeds]
        input_embeds_flat = torch.cat(
            [embed.flatten(2).transpose(1, 2) for embed in inputs_embeds], 1)
        spatial_shapes = torch.as_tensor(spatial_shapes_list,
                                         dtype=torch.long,
        ...
        for height, width in spatial_shapes_list[:-1]:
            level_start_index_list.append(level_start_index_list[-1] +
                                          height * width)
        split_sizes = [None] * self.num_feature_levels
        for i in range(self.num_feature_levels):
            if i < self.num_feature_levels - 1:
                split_sizes[i] = level_start_index_list[
                    i + 1] - level_start_index_list[i]
            else:
                split_sizes[
                    i] = last_hidden_state.shape[1] - level_start_index_list[i]

        encoder_output = torch.split(last_hidden_state, split_sizes, dim=1)

        # Compute final features
        outputs = [
            x.transpose(1, 2).view(batch_size, -1, spatial_shapes_list[i][0],
                                   spatial_shapes_list[i][1])
...
class Mask2FormerAttention(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states.permute(
            1, 0, 2) if hidden_states is not None else None
        position_embeddings = position_embeddings.permute(
            1, 0, 2) if position_embeddings is not None else None
        key_value_states = key_value_states.permute(
            1, 0, 2) if key_value_states is not None else None
        key_value_position_embeddings = (key_value_position_embeddings.permute(
            1, 0, 2) if key_value_position_embeddings is not None else None)
        ...
        if is_cross_attention:
            ...
            key_states = self._shape(self.k_proj(key_value_states), -1,
                                     batch_size)
            value_states = self._shape(self.v_proj(key_value_states_original),
                                       -1, batch_size)
        else:
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1,
                                     batch_size)
            value_states = self._shape(self.v_proj(hidden_states_original), -1,
                                       batch_size)

        proj_shape = (batch_size * self.num_heads, -1, self.head_dim)
        query_states = self._shape(query_states, target_len,
                                   batch_size).view(*proj_shape)
        ...
        if attn_weights.size() != (batch_size * self.num_heads, target_len,
                                   source_len):
            raise ValueError(
        ...
        if attention_mask is not None:
            if attention_mask.size() != (batch_size * self.num_heads,
        ...
        if output_attentions:
            ...
            attn_weights_reshaped = attn_weights.view(batch_size,
                                                      self.num_heads,
                                                      target_len, source_len)
        ...
        if attn_output.size() != (batch_size * self.num_heads, target_len,
                                  self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(batch_size, self.num_heads, target_len, self.head_dim)}, but is"
                f" {attn_output.size()}")

        attn_output = attn_output.view(batch_size, self.num_heads, target_len,
                                       self.head_dim)
...
class Mask2FormerMaskedAttentionDecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.cross_attn = nn.MultiheadAttention(
            self.embed_dim, self.config.num_attention_heads,
            self.config.dropout)
    ...
    def forward_post(
        ...
        hidden_states, cross_attn_weights = self.cross_attn(
            query=self.with_pos_embed(hidden_states,
                                      query_position_embeddings),
            key=self.with_pos_embed(encoder_hidden_states[level_index],
...
class Mask2FormerMaskedAttentionDecoder(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        # level embedding (3 scales) [1/32, 1/16. 1/8 scale features from the Pixel Decoder]
        self.num_feature_levels = 3
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            where = (attention_mask.sum(-1)
                     != attention_mask.shape[-1]).to(attention_mask.dtype)
        ...
        if not return_dict:
            outputs = [
                hidden_states, all_hidden_states, attentions, intermediate,
                intermediate_mask_predictions
            ]
...
class Mask2FormerMLPPredictionHead(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_dims, out_dims)):
            ...
            layer = Mask2FormerPredictionBlock(in_dim,
                                               out_dim,
...
class Mask2FormerMaskPredictor(nn.Module):
    ...
    def forward(self,
        ...
        outputs_mask = torch.einsum("bqc, bchw -> bqhw", mask_embeddings,
                                    pixel_embeddings)
        ...
        attention_mask = attention_mask.sigmoid().flatten(2).unsqueeze(
            1).repeat(1, self.num_heads, 1, 1)
    ...
    def forward(
        ...
        for i in range(self.num_feature_levels):
            ...
            multi_stage_positional_embeddings.append(
                self.position_embedder(multi_scale_features[i].shape,
            ...
            multi_stage_positional_embeddings[
                -1] = multi_stage_positional_embeddings[-1].permute(2, 0, 1)
...
class Mask2FormerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, Mask2FormerTransformerModule):
            if module.input_projections is not None:
                for input_projection in module.input_projections:
                    if not isinstance(input_projection, nn.Sequential):
                        init.xavier_uniform_(input_projection.weight,
                                             gain=xavier_std)
            ...
            thetas = torch.arange(
                module.n_heads,
                dtype=torch.int64).float() * (2.0 * math.pi / module.n_heads)
            grid_init = torch.stack([thetas.cos(), thetas.sin()], -1)
            grid_init = ((grid_init /
                          grid_init.abs().max(-1, keepdim=True)[0]).view(
                              module.n_heads, 1, 1,
                              2).repeat(1, module.n_levels, module.n_points,
                                        1))
            for i in range(module.n_points):
                grid_init[:, :, i, :] *= i + 1

            _copy_init_(module.sampling_offsets.bias, grid_init.view(-1))
            ...
            _copy_init_(module.empty_weight, empty_weight)
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        >>> url = (
        ...     "https://cdn-media.huggingface.co/Inference-API/"
        ...     "Sample-results-on-the-Cityscapes-dataset-The-above-"
        ...     "images-show-how-our-method-can-handle.png"
        ... )
        >>> with httpx.stream("GET", url) as response:
        ...     image = Image.open(BytesIO(response.read()))
        >>> inputs = image_processor(image, return_tensors="pt")
