"""
Training entrypoint for a tuned ConvNeXt + Mask2Former model.
"""
...
from train_utils import (build_dataloaders, build_training_components,
                         train_loop)
from utils import (IMAGE_PROCESSING_ROOT, cleanup_distributed, is_main_process,
                   load_goose_dataset_class, resolve_goose_data_root,
                   seed_everything, setup_distributed)

if str(IMAGE_PROCESSING_ROOT) not in sys.path:
    sys.path.insert(0, str(IMAGE_PROCESSING_ROOT))

ConvNeXtMask2FormerBoostedModel = importlib.import_module(
    "models").ConvNeXtMask2FormerBoostedModel


def main() -> None:
    # 1. Parse config and initialize runtime
    args = parse_args()
    ...
    # 4. Build model and training components
    id2label = {i: f"class_{i}" for i in range(args.num_classes)}
    label2id = {label: idx for idx, label in id2label.items()}

    model, optimizer, scaler, run_dir, epoch_log_path, curve_path = build_training_components(
        args=args,
        device=device,
        local_rank=local_rank,
        world_size=world_size,
        model_class=ConvNeXtMask2FormerBoostedModel,
        id2label=id2label,
        label2id=label2id,
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "[semantic_train.py] Unhandled exception on "
