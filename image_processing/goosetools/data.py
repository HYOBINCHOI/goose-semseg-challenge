...
def __check_labels(img_path: str,
                   lbl_path: str) -> Tuple[bool, Optional[List]]:
    """
    ...
    for label_type in ["color", "instanceids", "labelids"]:
        ...
        lbl_name = name + "_" + label_type + ".png"
...
def load_splits(src_path: str,
                splits: List[str] = ["train", "val"]) -> Tuple[List[Dict]]:
    """
    Parameters:
    ...
    for c in splits:
        ...
        datadicts.append(
            process_goose_folder(os.path.join(img_path, c),
...
class GOOSE_Dataset(Dataset):
    ...
    def __init__(
        ...
        Parameters:
            ...
            resize_size   [Iter]    : List with the target resize size of the images
                                      (After the crop if crop == True) (width, height)
        ...
        if crop:
            self.crop_ratio = (crop_ratio if crop_ratio is not None else
    ...
    def get_images(self, index):
        ...
        label = Image.open(
            self.dataset_dict[index]["semantic_path"]).convert("L")

        color = None
        instances = None
        if (self.dataset_dict[index]["color_path"]):
            color = Image.open(
                self.dataset_dict[index]["color_path"]).convert("RGB")
        if (self.dataset_dict[index]["instance_path"]):
            instances = Image.open(
                self.dataset_dict[index]["instance_path"]).convert("L")
...
if __name__ == "__main__":
    splits = GOOSE_Dataset.splits_from_path(
        "/home/miguel/datasets/goose/goose2d")
    for s in splits:
