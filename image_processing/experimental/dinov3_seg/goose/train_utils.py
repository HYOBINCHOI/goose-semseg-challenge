...
from checkpoint_utils import (RollingCheckpointManager, append_epoch_log,  # noqa: E402
                              ensure_epoch_log_file, save_checkpoint,
                              save_training_curves)
from dataset_utils import GooseMask2FormerCollator, move_batch_to_device  # noqa: E402
...
def outputs_to_semantic_predictions(
    ...
    class_probs = torch.softmax(outputs.class_queries_logits, dim=-1)[..., :-1]
    mask_probs = torch.sigmoid(outputs.masks_queries_logits)
    semantic_logits = torch.einsum(
        "bqc,bqhw->bchw",
        class_probs.to(torch.float32),
        mask_probs.to(torch.float32),
    )
...
def build_optimizer(args: argparse.Namespace, model: nn.Module) -> AdamW:
    ...
    encoder_markers = (
        ...
        "segmentation_backbone.backbone",
    )

    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        if any(marker in name for marker in encoder_markers):
...
def maybe_enable_gradient_checkpointing(model: nn.Module) -> None:
    ...
    if mask2former is not None and hasattr(
        ...
        try:
            ...
            print(
                "Warning: Mask2Former does not support "
                f"gradient checkpointing. Original error: {exc}"
            )
...
def run_epoch(
    ...
    for step, batch in enumerate(progress_bar, start=1):
        ...
        predictions = outputs_to_semantic_predictions(
            outputs,
            target_size=batch["semantic_maps"].shape[-2:],
        )
    ...
    if dist.is_available() and dist.is_initialized():
        count_tensor = torch.tensor(total_batches, dtype=torch.float64, device=device)
...
def resume_if_needed(
    ...
    if missing:
        print(f"[INFO] Missing keys while resuming: {missing}")
    if unexpected:
        print(f"[INFO] Unexpected keys while resuming: {unexpected}")
...
def build_dataloaders(
    ...
    val_sampler = DistributedSampler(
        val_dataset,
        shuffle=False,
    ) if world_size > 1 and args.dist_eval else None
...
def train_loop(
    ...
    rolling_ckpt = (
        RollingCheckpointManager(run_dir, max_keep=2) if is_main_process() else None
    )
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            miou_improved = val_miou > (
                best_val_miou + args.early_stopping_min_delta
            )
            ...
            if world_size > 1:
                loss_tensor = torch.tensor(best_val_loss, dtype=torch.float64, device=device)
                miou_tensor = torch.tensor(best_val_miou, dtype=torch.float64, device=device)
            ...
            if is_main_process():
                if rolling_ckpt is not None and rolling_ckpt.should_save(val_miou):
                    rolling_ckpt.save(
                        model,
                        optimizer,
                        scaler,
